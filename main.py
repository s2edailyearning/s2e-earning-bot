...
BINARY_BONUS_PER_PAIR = 50  # Each left+right match = Rs50
BINARY_BONUS_PREMIUM_PER_PAIR = 100  # Premium user gets Rs100 per pair
...
def is_valid_upi_format(upi):
    ...
    if not re.match(r'^[a-zA-Z0-9._\-]{2,256}$', username):
        return False, "Username can only have letters, numbers, . - _"
    if not re.match(r'^[a-zA-Z]{2,64}$', bank):
        return False, "Bank name can only have letters after @"
...
def validate_upi_auto(upi):
    return is_valid_upi_format(upi)

# === AUTO TASK VERIFICATION (Added) ===
CHANNEL_USERNAME_FOR_AUTO = "@s2edayincome"

async def auto_verify_channel_join(uid, context):
    try:
        chat_member = await context.bot.get_chat_member(chat_id=CHANNEL_USERNAME_FOR_AUTO, user_id=uid)
...
async def auto_verify_task(uid, task, context):
    ...
    if 'channel' in task_title or 't.me' in task_link or 's2edayincome' in task_link.lower():
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌳 Binary Team", callback_data="binary_team"), InlineKeyboardButton("💎 Support Plans", callback_data="support_plans")],
...
async def binary_team_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def my_ref_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(f"Referrals Active: {cnt} Binary L{left} R{right} Pairs {pairs} Earnings Rs{earnings} Link: {ref_link}\nLeft+Right match = Bonus!", reply_markup=main_menu())
...
async def get_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_valid:
        await update.message.reply_text(f"Invalid UPI! {msg} Example: yourname@upi, 9876543210@ybl Try again:")
        return UPI
    users_db[uid]['upi']=upi
    await update.message.reply_text(f"UPI Verified! {upi} is valid! Pincode:")
    return PINCODE
...
async def get_profession(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Menu: Binary Team available! Left-Right matching bonus!", reply_markup=main_menu())
    return ConversationHandler.END
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(binary_team_cb, pattern="^binary_team$"))
    app.add_handler(CallbackQueryHandler(binary_history_cb, pattern="^binary_history$"))
