...
def is_valid_upi_format(upi):
    # UPI format: username@bank - 2-256 chars before @, 2-64 after @
    # Valid chars: a-z A-Z 0-9 . - _
    if not upi or "@" not in upi:
        return False, "UPI must contain @ symbol (example: yourname@upi)"
    parts = upi.split("@")
    if len(parts) != 2:
        return False, "Invalid UPI - only one @ allowed"
    username, bank = parts
    if len(username) < 2 or len(username) > 256:
        return False, "Username part 2-256 chars needed before @"
    if len(bank) < 2 or len(bank) > 64:
        return False, "Bank part 2-64 chars needed after @"
    # Check valid chars
    if not re_module.match(r'^[a-zA-Z0-9._\-]{2,256}$', username):
        return False, "Username can only have letters, numbers, . - _"
    if not re_module.match(r'^[a-zA-Z]{2,64}$', bank):
        return False, "Bank name can only have letters (2-64 chars) after @"
    # Common UPI banks check (optional)
    valid_banks = ['upi', 'okaxis', 'okhdfcbank', 'okicici', 'oksbi', 'ybl', 'ibl', 'axl', 'sbi', 'hdfc', 'icici', 'paytm', 'phonepe', 'gpay', 'airtel', 'jio']
    # Allow any bank but warn if unknown
    if bank.lower() not in valid_banks:
        # Still allow but with warning - many banks exist
        pass
    # Blacklisted or fake UPIs
    if ".." in upi or "__" in upi or "--" in upi:
        return False, "Invalid UPI - consecutive special chars not allowed"
    if upi.lower().startswith('.') or upi.lower().endswith('.'):
        return False, "UPI cannot start or end with special char"
    return True, "Valid UPI"

def validate_upi_auto(upi):
    is_valid, msg = is_valid_upi_format(upi)
    return is_valid, msg

# === AUTO TASK VERIFICATION ===
async def auto_verify_channel_join(uid, context):
    # Auto check if user joined sponsor channel
    try:
        chat_member = await context.bot.get_chat_member(chat_id=CHANNEL_USERNAME, user_id=uid)
        status = chat_member.status
        if status in ['member', 'administrator', 'creator', 'owner']:
            return True, f"Verified - User is {status} in channel"
        else:
            return False, f"Not joined - Status {status}"
    except Exception as e:
        # If bot not admin in channel, cannot verify
        return None, f"Cannot auto verify - Bot not admin in channel: {e}"

async def auto_verify_task(uid, task, context):
    # Try auto verification based on task type
    task_title = task.get('title','').lower()
    task_link = task.get('link','')
    
    # For channel join tasks
    if 'channel' in task_title or CHANNEL_USERNAME.replace('@','') in task_link or 't.me' in task_link:
        result, msg = await auto_verify_channel_join(uid, context)
        if result is True:
            return True, "Auto Verified - Channel Joined!"
        elif result is False:
            return False, "Auto Check Failed - Please join channel first!"
        else:
            return None, "Manual verification needed"
    
    # For other tasks, cannot auto verify - need manual screenshot
    return None, "Manual verification needed - Screenshot required"
...
async def handle_screenshot_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.photo:
        await update.message.reply_text("Please send as PHOTO, not file! Screenshot ni photo ga pampu!")
        return UPLOAD_SCREENSHOT
    # Time limit check
    open_time = task_open_time.get(uid)
    if open_time:
        mins_passed = (datetime.now() - open_time).total_seconds() / 60
        if mins_passed > 15:
            await update.message.reply_text(f"Time over! 15 mins lo pampali! Malli task open cheyyi!", reply_markup=main_menu())
            return ConversationHandler.END
    photo = update.message.photo[-1]
    file_id = photo.file_id
    file_unique_id = photo.file_unique_id
    # Duplicate check - 3 warnings system
    if file_unique_id in screenshot_hashes:
        ...
        if count == 1:
            await update.message.reply_text(f"WARNING 1/3 - Same Screenshot Found! Ee screenshot ni inkokaru already use chesaru! Same screenshot share chesukunatlu undi! 1st Warning!", reply_markup=main_menu())
            return ConversationHandler.END
        elif count == 2:
    ...
    # === AUTO TASK VERIFICATION - Try auto before manual ===
    try:
        auto_result, auto_msg = await auto_verify_task(uid, task, context)
        if auto_result is True:
            # Auto verified - direct approve
            ...
            if task.get('reward',5) != 5:
                bonus_balance[uid]=bonus_balance.get(uid,0)+(task.get('reward',5)-5)
            ...
            await update.message.reply_text(f"AUTO VERIFIED! {auto_msg} Task {task.get('title')} Approved +Rs{task.get('reward',5)} Balance Rs{get_balance(uid)} No admin wait!", reply_markup=main_menu())
            for admin_id in ADMIN_ID_LIST:
                try:
                    await context.bot.send_message(chat_id=admin_id, text=f"AUTO VERIFIED Task! User {users_db.get(uid,{}).get('name')} ID {uid} Task {task.get('title')} Auto Approved Rs{task.get('reward',5)}")
                except: pass
            return ConversationHandler.END
        elif auto_result is False:
            await update.message.reply_text(f"Auto Verification Failed! {auto_msg} Please join channel first!", reply_markup=main_menu())
            return ConversationHandler.END
    except Exception as e:
    ...
    await update.message.reply_text(f"Screenshot Received! Task {task.get('title')} Pending Admin Verification! Admin check: Name matches? Original? Not duplicate?", reply_markup=main_menu())
    for admin_id in ADMIN_ID_LIST:
        try:
...
async def get_upi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_valid, msg = validate_upi_auto(upi)
    if not is_valid:
        await update.message.reply_text(f"❌ Invalid UPI! {msg}\nExample: yourname@upi, 9876543210@ybl, name@okaxis\nTry again:")
        return UPI
    users_db[uid]['upi']=upi
    await update.message.reply_text(f"✅ UPI Verified! {upi} is valid!\nPincode:")
    return PINCODE
