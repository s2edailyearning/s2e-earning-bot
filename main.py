...
@app_flask.route('/')
def home(): return f"S2E V35 Final Fixed Live Admins:{config.get('admins')} Users:{len(users_data)}"
...
async def button_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
  ...
  if data.startswith("task_"):
    ...
    if tid not in get_completed(uid):
      ...
      add_commission(uid,"daily_work",10)
    await query.answer("Done!")
    return
  if data=="main_menu":
    # Re-trigger start via message
    await query.message.reply_text("Use /start")
    return
  ...
  if data=="backup":
    try:
      ...
      save_json(USERS_FILE, users_data)
      save_json(REFERRAL_FILE, referrals)
      await query.message.reply_document(document=open(USERS_FILE,'rb'), filename="users_progress.json", caption="Users Backup - Save to Drive!")
      await query.message.reply_document(document=open(REFERRAL_FILE,'rb'), filename="referrals.json", caption="Referrals Backup - L1/L2 data!")
      await query.message.reply_document(document=open(CONFIG_FILE,'rb'), filename="bot_config.json", caption="Config Backup - Admins + Settings!")
      await query.edit_message_text("✅ All 3 Backup files sent! Save in Google Drive. If bot deleted, upload these to new deployment - total users will restore!")
    except Exception as e:
...
async def backup_cmd(update:Update, context:ContextTypes.DEFAULT_TYPE):
  ...
  try:
    ...
    save_json(USERS_FILE, users_data)
    save_json(REFERRAL_FILE, referrals)
    await update.message.reply_document(document=open(USERS_FILE,'rb'), filename="users_progress.json")
    await update.message.reply_document(document=open(REFERRAL_FILE,'rb'), filename="referrals.json")
    await update.message.reply_document(document=open(CONFIG_FILE,'rb'), filename="bot_config.json")
    await update.message.reply_text("✅ All 3 Backup files - Save to Drive! If bot deleted, upload these to new bot - total users will restore! Backup + Referral L1/L2 system ready!")
  except Exception as e:
...
def main():
    if not BOT_TOKEN:
        print("TOKEN missing")
        return
    print("Waiting 15 sec for old instance to shutdown to avoid Conflict...")
    time.sleep(15)
    ...
    print("V35 Final Starting - Backup fixed, Referral L1 10%+2% L2 0.2%")
