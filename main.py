...
# Optional single-admin environment variable (useful on Render/Railway/etc.)
_single_admin = os.getenv("ADMIN_ID", "").strip()
if _single_admin.isdigit() and int(_single_admin) not in ADMIN_ID_LIST:
    ADMIN_ID_LIST.append(int(_single_admin))
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show the admin panel from either a normal command Update or a callback query.

    This is deliberately defensive because the Back-to-Admin callback previously
    passed CallbackQuery directly into this function, while the function expected
    Update.message. That could silently fail the admin navigation.
    """
    try:
        ...
        if not is_admin(uid):
            text = f"❌ You are not an admin.\n\nYour Telegram ID: {uid}\n\nSet this ID in ADMIN_IDS (or ADMIN_ID) on your hosting panel, then restart the bot."
            if hasattr(update, "message") and update.message:
                await update.message.reply_text(text)
            elif hasattr(update, "message") and update.message is None and hasattr(update, "answer"):
                await update.answer()
            else:
        ...
        pending_withdraw = len([w for w in withdraw_requests.values() if w.get('status') == 'processing'])
        msg = "🔐 ADMIN PANEL - S2E Ultimate + Poster\n\n"
        msg += f"👥 Users: {len(users_db)}\n"
        msg += f"📋 Pending Daily: {len(pending_daily)}\n"
        msg += f"💰 Pending Withdraw: {pending_withdraw}\n"
        ...
        msg += "Commands:\n"
        msg += "/add_task open close next title link reward\n"
        msg += "/set_task_image <id> - Then send poster image!\n"
        msg += "Example: /add_task 12:45PM 15min 1:03PM Task 3 Google Review https://maps.app.goo.gl/xxx 5\n"
        msg += "Then /set_task_image 1 + send TASK 3 poster!\n\n"
        msg += "/list_tasks /list_promos /skipped all /warnings /banned"

        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"📋 Pending Daily ({len(pending_daily)})", callback_data="admin_view_pending"), InlineKeyboardButton(f"💰 Withdraw ({pending_withdraw})", callback_data="admin_view_withdraw")],
        ...
        elif hasattr(update, "message") and update.message is None and hasattr(update, "edit_message_text"):
            await update.edit_message_text(msg[:4000], reply_markup=kb)
        else:
            await context.bot.send_message(chat_id=uid, text=msg[:4000], reply_markup=kb)
    except Exception as e:
        print(f"ADMIN PANEL ERROR: {e}")
        try:
            uid = getattr(getattr(update, "effective_user", None), "id", None) or getattr(getattr(update, "from_user", None), "id", None)
            if uid:
                await context.bot.send_message(chat_id=uid, text=f"⚠️ Admin panel error: {e}")
        except Exception:
            pass
