...
# Contact Admin is intentionally fixed to the public username.
# Never fall back to a Telegram numeric-ID deep link.
SUPPORT_USERNAME = "@s2edayincome"
CONTACT_USERNAME = "@s2edayincome"
CONTACT_ADMIN_ID = 7256515560
...
def get_user_plan_id(uid):
    """Return the CURRENT 5-plan ID: 0=Free, 1=Starter, 2=Pro, 3=Elite, 4=VIP.
    Saved activation plan_id is authoritative; name mapping is only a legacy fallback.
    """
    try:
        ...
        if isinstance(record, dict):
            ...
            p = str(record.get('plan_name') or record.get('name') or record.get('plan') or 'free').strip().lower()
        else:
            p = str(record).strip().lower()
        if p in ('free', '0', 'no plan'):
            return 0
        if p in ('starter', 'basic', '199', '499', '1'):
            return 1
        if p in ('pro', 'premium', '999', '1999', '2'):
            return 2
        if p in ('elite', '3', '4999'):
            return 3
        if p in ('vip', '4', '9999'):
            return 4
        try:
            return int(p) if int(p) in (0, 1, 2, 3, 4) else 0
        except Exception:
            return 0
    except Exception:
        return 0
...
def get_task_reward_for_user(task, uid):
    """Return the task's plan-wise reward when supplied; otherwise use its fixed reward."""
    try:
    ...
    for task in today_tasks:
        ...
        if audience == 'all' or audience == 'all':
            ...
            if audience == 0 or audience == plan_id:
                filtered.append(task)
        elif str(audience).lower() in ['1', 'starter', 'basic', '199', '499']:
            if plan_id in [1,2,3,4]:
                filtered.append(task)
        elif str(audience).lower() in ['2', 'pro', 'premium', '999', '1999']:
            if plan_id in [2,3,4]:
                filtered.append(task)
        elif str(audience).lower() in ['3', 'elite', '4999']:
            if plan_id in [3,4]:
                filtered.append(task)
        elif str(audience).lower() in ['4', 'vip', '9999']:
            if plan_id == 4:
                filtered.append(task)
        else:
            filtered.append(task)
...
async def admin_approve_plan_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Keep a separate immutable activation ledger for date-wise admin reports.
        # This does not replace or alter the current user plan snapshot.
        plan_activation_history_db.append({
            "uid": int(uid),
            "name": str((users_db.get(uid, {}) or {}).get("name") or (users_db.get(str(uid), {}) or {}).get("name") or "Unknown"),
...
# Fixed Product Promotion rewards by CURRENT plan ID. Admin does not enter rewards.
PRODUCT_PROMOTION_REWARDS = {0: 10, 1: 30, 2: 80, 3: 200, 4: 500}
...
# Persistent history of every approved Support Plan activation.
...
def _product_reward_for_user(task, uid):
    """Product Promotion reward is ALWAYS fixed by the user's current plan."""
    pid = get_user_plan_id(uid)
    return int(PRODUCT_PROMOTION_REWARDS.get(pid, PRODUCT_PROMOTION_REWARDS[0]))
...
async def get_dob(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📱 Mobile Number\n\n"
        "For correct account details, tap the button below and share the SAME Telegram account phone number.\n"
        "If you cannot share it, you may type a 10-digit number.",
        reply_markup=ReplyKeyboardMarkup([[KeyboardButton("📱 Share Telegram Number", request_contact=True)]], resize_keyboard=True, one_time_keyboard=True)
    )
    return MOBILE
...
async def get_mobile_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save the phone number shared through Telegram's Contact button.

    This is stored only in the member record/admin details. Referral screens
    intentionally never display phone numbers.
    """
    uid = update.effective_user.id
    contact = getattr(update.message, "contact", None)
    if not contact or not contact.phone_number:
        await update.message.reply_text("❌ Could not read the Telegram contact. Please share your own contact again.")
        return MOBILE
    phone = str(contact.phone_number).strip()
    # Telegram Contact is the authoritative number; don't let a manually typed
    # number overwrite it during this registration step.
    users_db.setdefault(uid, {})["mobile"] = phone
    users_db[uid]["telegram_phone"] = phone
    await update.message.reply_text("✅ Telegram phone number saved.\n\nUPI ID? Example: yourname@upi")
    return UPI
...
async def admin_product_promo_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if live:
        ...
        msg = (
            ...
            "Fixed rewards: Free ₹10 | Starter ₹30 | Pro ₹80 | Elite ₹200 | VIP ₹500\n\n"
...
async def set_shop_product_image_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prod:
        ...
        return
    context.user_data["awaiting_shop_product_image"] = pid
    await update.message.reply_text(
        f"🖼️ Ready for Product ID {pid}: {prod.get('name','')}\n"
        "Now send the product image as a PHOTO."
    )

async def shop_product_image_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save the Telegram file_id of the next photo for the selected product."""
    if not is_admin(update.effective_user.id):
        return
    pid = context.user_data.get("awaiting_shop_product_image")
    if not pid:
        return
...
async def add_product_promo_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Create an OWN Product Promotion campaign using fixed plan-wise rewards.

    Admin only enters: download deadline, screenshot window, title and instructions.
    Rewards are always Free ₹10 / Starter ₹30 / Pro ₹80 / Elite ₹200 / VIP ₹500.
    """
    uid = update.effective_user.id
    if not is_admin(uid):
    ...
    if not text:
        await update.message.reply_text(
            "Usage:\n/add_product_promo DOWNLOAD_DEADLINE SCREENSHOT_OPEN SCREENSHOT_CLOSE TITLE | INSTRUCTIONS\n\n"
            "Example:\n/add_product_promo 10:00AM 8:00PM 10:00PM My Product | Download the video, put it on WhatsApp Status and send screenshot during 8PM-10PM.\n\n"
            "Fixed reward: Free ₹10 | Starter ₹30 | Pro ₹80 | Elite ₹200 | VIP ₹500\n\n"
            "Then send the promotion VIDEO to this bot."
        )
        return
    ...
    if '|' in text:
        ...
        desc = desc.strip() or "Download the video and put it on your WhatsApp Status. Send the screenshot only during the scheduled screenshot window."
    else:
        left = text
        desc = "Download the video and put it on your WhatsApp Status. Send the screenshot only during the scheduled screenshot window."

    parts = left.split()
    if len(parts) < 4:
        await update.message.reply_text(
            "❌ Invalid format.\n\n"
            "Use: /add_product_promo 10:00AM 8:00PM 10:00PM My Product | Instructions\n\n"
            "Fixed reward: Free ₹10 | Starter ₹30 | Pro ₹80 | Elite ₹200 | VIP ₹500"
        )
        return

    download_deadline, shot_open, shot_close = parts[:3]
    title = ' '.join(parts[3:]).strip() or 'Product Promotion'
    try:
        d = parse_time_str(download_deadline)
        so = parse_time_str(shot_open)
        sc = parse_time_str(shot_close)
        if not d or not so or not sc:
            raise ValueError('Invalid time')
    except Exception:
        await update.message.reply_text("❌ Invalid time. Use examples 10:00AM 8:00PM 10:00PM")
        return

    global product_promo_counter
    today = str(get_ist_today())
    # Only one live OWN Product Promotion campaign is allowed for today.
    ...
    rewards = dict(PRODUCT_PROMOTION_REWARDS)
    task = {
        'id': product_promo_counter,
        'date': today,
        'title': title,
        'description': desc,
        'download_deadline': d.strftime('%H:%M'),
        'screenshot_open': so.strftime('%H:%M'),
        'screenshot_close': sc.strftime('%H:%M'),
        'reward': rewards[0],
        'rewards': rewards,
        'video_file_id': None,
    ...
    save_data()
    await update.message.reply_text(
        f"✅ Product Promotion ID {task['id']} created.\n\n"
        f"🎥 Now send the promotion VIDEO to this bot.\n\n"
        f"Download deadline: {task['download_deadline']}\n"
        f"Screenshot: {task['screenshot_open']} → {task['screenshot_close']}\n\n"
        "💰 FIXED REWARDS\n"
        "🆓 Free: ₹10\n"
        "🥉 Starter: ₹30\n"
        "💎 Pro: ₹80\n"
...
async def product_approve_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    campaign = next((x for x in product_promo_db if isinstance(x, dict) and int(x.get('id', -1)) == tid), None)
    reward = _product_reward_for_user(campaign or {}, uid)
...
def get_contact_username():
    # Always use the configured public username.
    return "@s2edayincome"

def get_contact_url(message_text=None):
    # IMPORTANT: never use tg://user?id=... here.
    # Always open the public Telegram username so the user's phone/account
    # number is not exposed by a numeric-ID deep link.
    base = "https://t.me/s2edayincome"
    if message_text:
...
async def set_contact_username_cmd(update, context):
    if not is_admin(update.effective_user.id):
        return
    await update.message.reply_text(
        "✅ Contact Admin is fixed to @s2edayincome\n"
        "Users will always be sent to: https://t.me/s2edayincome"
    )
...
async def missed_tasks_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb=[]
    for t in missed:
        per_reward = get_task_reward_for_user(t, uid)
        msg += (f"Task {t.get('task_number','?')}: {t.get('title','')}\n"
                f"Time: {t.get('open_time','')} → {t.get('close_time','')} | Reward: ₹{per_reward}\n"
                f"Link: {t.get('link','')}\n\n")
        tid=t.get('id')
        st=user_task_status.get(uid,{}).get(tid,{})
        st=st.get('status') if isinstance(st,dict) else st
...
async def assign_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        duration = int(plan.get('duration', plan.get('validity_days', 30)) or 30)
        daily = int(plan.get('daily_limit', plan.get('daily_task_limit', 10)) or 10)
        price = int(plan.get('price', 0) or 0)
        name = str(plan.get('name', 'Plan'))
        expiry = get_ist_today() + timedelta(days=duration)
        ...
        old_name = old_plan.get('plan_name','No Plan') if isinstance(old_plan, dict) else 'No Plan'
...
async def userlist_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if context.args:
            ...
            if raw_filter == "today":
                ...
                if date_filter is None:
                    await update.message.reply_text(
                        "❌ Invalid date. Use /userlist today, /userlist yesterday, or /userlist YYYY-MM-DD"
                    )
                    return
        ...
        def referrer_text(member_uid):
            parent = referral_map.get(member_uid) or referral_map.get(str(member_uid))
            if parent in (None, "", 0, "0"):
                return "None / Direct Join"
            try:
                parent_id = int(parent)
            except Exception:
                return f"Unknown | ID: {parent}"
            pdata = users_db.get(parent_id) or users_db.get(str(parent_id)) or {}
            pname = pdata.get("name") if isinstance(pdata, dict) else None
            return f"{str(pname or 'Unknown User').strip()} | ID: {parent_id}"
        ...
        for uid, data, join_date in active_items[:50]:
            ...
            lines.append(
                ...
                f"💎 Plan: {plan_name}\n"
                f"👥 Referred By: {referrer_text(uid)}\n"
                f"📅 Joined: {join_date}\n"
...
async def plan_activations_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin date-wise Support Plan activation report.

    Usage: /plan_activations [YYYY-MM-DD|today|yesterday]
    Shows activated member name, plan, amount and activation time, plus total amount.
    """
    if not is_admin(update.effective_user.id):
        return
    raw = str(context.args[0]).strip().lower() if context.args else "today"
    if raw == "today":
        target = get_ist_today()
    elif raw == "yesterday":
        target = get_ist_today() - timedelta(days=1)
    else:
        try:
            target = date.fromisoformat(raw)
        except Exception:
            await update.message.reply_text("❌ Invalid date. Use /plan_activations YYYY-MM-DD, today or yesterday")
            return

    rows = []
    for row in plan_activation_history_db if isinstance(plan_activation_history_db, list) else []:
        if str(row.get("date", ""))[:10] == str(target):
            rows.append(row)

    # Backward-compatible fallback: existing current plan snapshots may have
    # an activation date even when they predate the new immutable history ledger.
    if not rows:
        for uid, raw_plan in list(user_plans.items()):
            try:
                uid_i = int(uid)
            except Exception:
                continue
            if not isinstance(raw_plan, dict):
                continue
            d = str(raw_plan.get("date") or raw_plan.get("activated_at") or "")[:10]
            if d != str(target):
                continue
            user = users_db.get(uid_i, {}) or users_db.get(str(uid_i), {}) or {}
            rows.append({
                "uid": uid_i,
                "name": str(user.get("name") or "Unknown"),
                "plan_name": str(raw_plan.get("plan_name") or raw_plan.get("plan") or "Plan"),
                "price": float(raw_plan.get("price", 0) or 0),
                "activated_at": str(raw_plan.get("activated_at") or raw_plan.get("date") or ""),
            })

    rows.sort(key=lambda r: str(r.get("activated_at", "")))
    total = sum(float(r.get("price", 0) or 0) for r in rows)
    if not rows:
        await update.message.reply_text(f"💎 PLAN ACTIVATIONS — {target}\n\nNo Support Plan activations found for this date.")
        return

    lines = [f"💎 PLAN ACTIVATIONS — {target}", "", f"👥 Activations: {len(rows)}", f"💰 Total Activation Amount: ₹{total:.2f}", ""]
    for i, row in enumerate(rows, 1):
        ts = str(row.get("activated_at", "")).replace("T", " ")[:19]
        lines.append(f"{i}. 👤 {str(row.get('name') or 'Unknown')[:40]}")
        lines.append(f"   💎 Plan: {row.get('plan_name') or 'Plan'}")
        lines.append(f"   💰 Amount: ₹{float(row.get('price', 0) or 0):.2f}")
        if ts:
            lines.append(f"   🕒 Activated: {ts}")
        lines.append("")
    await update.message.reply_text("\n".join(lines)[:4000])
...
async def userdetails_cmd(update, context):
    """Admin: show complete profile, plan, referrer, L1 and L2 member details for one user."""
    if not is_admin(update.effective_user.id):
        return
    if not context.args:
    ...
    def user_display(uid):
        ...
        if not isinstance(data, dict):
            data = {}
        name_ = str(data.get("name") or "Unknown User").strip()
        username_ = str(data.get("username") or "").strip()
        if username_ and not username_.startswith("@"):
            username_ = "@" + username_
        plan_ = _get_user_plan_record(uid) or {}
        plan_name_ = str(plan_.get("plan_name") or plan_.get("name") or plan_.get("plan") or "Free / No Plan")
        return name_, username_ or "Not set", plan_name_
    ...
    parent = referral_map.get(target) or referral_map.get(str(target))
    if parent not in (None, "", 0, "0"):
        try:
            parent = int(parent)
            ref_name, ref_username, ref_plan = user_display(parent)
            referred_by = f"{ref_name} | ID: {parent} | {ref_username} | {ref_plan}"
        except Exception:
            referred_by = f"Unknown | ID: {parent}"
    else:
        referred_by = "None / Direct Join"

    l1, l2 = get_referral_chain(target)
    l1 = [int(x) for x in l1 if str(x).lstrip("-").isdigit()]
    l2 = [int(x) for x in l2 if str(x).lstrip("-").isdigit()]
    ...
    msg = (
        ...
        f"🟢 L1 Direct Referrals: {len(l1)}\n"
        f"🔵 L2 Referrals: {len(l2)}"
    )
    await update.message.reply_text(msg[:4000])

    def referral_lines(title, members):
        lines = [title]
        if not members:
            lines.append("None")
        else:
            for i, member_id in enumerate(members, 1):
                n, un, pl = user_display(member_id)
                lines.append(f"{i}. 👤 {n}\n   🆔 ID: {member_id}\n   🔹 Username: {un}\n   💎 Plan: {pl}")
        return lines

    # Send L1/L2 in separate chunks so a large referral network is not truncated.
    for title, members in ((f"🟢 L1 MEMBER LIST — {len(l1)}", l1), (f"🔵 L2 MEMBER LIST — {len(l2)}", l2)):
        lines = referral_lines(title, members)
        chunk = ""
        for line in lines:
            add = ("\n" if chunk else "") + line
            if len(chunk) + len(add) > 3900:
            ...
            try:
                ...
                for part in parts:
                    if ':' in part:
                        ...
                        if k in ['free', '0']:
                            rewards_dict[0] = v
                        elif k in ['1', 'starter', 'basic', '199', '499']:
                            rewards_dict[1] = v
...
async def add_bulk_tasks_cmd(update, context):
    ...
    if not full_text:
        await update.message.reply_text(
            ...
            "16:00 17:00 ProTask https://t.me/... 20 starter\n\n"
            "Amount separation:\n"
            "• all/free = Free + All paid users can do\n"
            "• starter/pro/elite/vip = Only that plan & higher\n"
    ...
    try:
        ...
        if len(args) < 5:
            await update.message.reply_text(
                ...
                "Audience: all, free, 0,1,2,3,4, starter, pro, elite, vip\n"
                "Ex: /add_task_manual 10:00 11:00 Title https://t.me/... 10 pro"
            )
            return
        ...
        try:
            ...
            conv_reg = ConversationHandler(
                ...
                states={
                    ...
                    MOBILE:[MessageHandler(filters.CONTACT, get_mobile_contact), MessageHandler(filters.TEXT & ~filters.COMMAND, get_mobile)],
            ...
            async def v56_task_image_simple_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
                try:
                    ...
                    # latest task. Skip ONLY the exact message already consumed by broadcast.
                    if context.user_data.get("broadcast_handled_message_id") == getattr(update.effective_message, "message_id", None):
                        return
            ...
            async def fallback_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
                try:
                    ...
                    if is_admin(uid) and context.user_data.get('awaiting_shop_product_image'):
                        print(f" fallback_photo_handler: Admin {uid} has awaiting_shop_product_image {context.user_data.get('awaiting_shop_product_image')} - Handling as shop product image!")
                        await shop_product_image_photo_handler(update, context)
                        return
            ...
            async def daily_open_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                await q.message.reply_text(text, reply_markup=kb)
            async def shopping_requirement_guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
                """Block earning/promotion task panels until the current shopping target is met.

                Shopping, wallet and account pages remain available so the member can
                purchase and see exactly what is required.
                """
                q = update.callback_query
                if not q:
                    return
                ...
                await q.message.reply_text(
                    "🛒 PURCHASE REQUIRED\n\n"
                    f"💸 Approved Withdrawal: ₹{st.get('withdrawn',0):g}\n"
                    f"🎯 Required Purchase (20%): ₹{st.get('required',0):g}\n"
                    f"🛍️ Purchased: ₹{st.get('purchased',0):g}/₹{st.get('required',0):g}\n"
                    f"⏳ Remaining: ₹{st.get('pending',0):g}\n\n"
                    "Please complete the required shopping purchase. Once the target is reached, "
                    "Daily Tasks, Scheduled Tasks, Promo Tasks, Product Promotion and Promote My Shop access will return automatically.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🛒 Shopping", callback_data="shopping")],
                        [InlineKeyboardButton("🛍️ Shopping Progress", callback_data="shopping_progress")],
                        [InlineKeyboardButton("🏠 Menu", callback_data="back_menu")],
                    ])
                )
                raise ApplicationHandlerStop
            ...
            app.add_handler(CallbackQueryHandler(
                shopping_requirement_guard,
                pattern=r"^(daily|scheduled|scheduled_tasks|promo_tasks|product_promo|promote_shop)$"
            ), group=-3)
            ...
            app.add_handler(CommandHandler("plan_activations", plan_activations_cmd))
            ...
            app.add_handler(CommandHandler("user_info", userdetails_cmd))
...
async def broadcast_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    BROADCAST_PENDING_ADMINS.discard(uid)
    BROADCAST_ACTIVE_SENDING.add(uid)  # V63 compatibility marker
    # V67 FIX: Mark this exact update so lower-priority PHOTO handlers cannot
    # mistake a broadcast photo for a task poster. This marker is per-user and
    # per-message, so later unrelated photos continue to work normally.
    context.user_data["broadcast_handled_message_id"] = getattr(msg, "message_id", None)
